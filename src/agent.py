...
#         # Financial metrics for better parsing
#         self.metric_aliases = {
#             'total revenue': 'revenue', 'revenues': 'revenue', 'sales': 'revenue',
#             'operating margin': 'operating margin', 'operating income': 'operating income',
#             'gross margin': 'gross margin', 'gross profit': 'gross profit',
...
#     def parse_query(self, query: str) -> Dict[str, Any]:
#         """Enhanced query parsing to identify intent and components"""
#         query_lower = query.lower()
...
#         # IMPROVED: Better detection of multi-company queries
#         multi_company_indicators = [
#             'compare', 'which', 'highest', 'lowest', 'across', 'between', 
#             'all three', 'each company', 'three companies', 'companies'
#         ]
...
#             companies = ['MSFT', 'GOOGL', 'NVDA']
#         elif 'each company' in query_lower or 'three companies' in query_lower:
#             # Force all three companies even if some are mentioned
#             companies = ['MSFT', 'GOOGL', 'NVDA']
        
#         # Extract years
#         years = list(set(re.findall(r'(202[0-9])', query)))
        
#         # Handle year ranges like "from 2022 to 2023"
#         year_range_match = re.search(r'from\s+(202[0-9])\s+to\s+(202[0-9])', query_lower)
#         if year_range_match:
#             years = [year_range_match.group(1), year_range_match.group(2)]
...
#         query_type = self._determine_query_type(query_lower, companies, years, metrics)
...
#     def _determine_query_type(self, query_lower: str, companies: List[str], years: List[str], metrics: List[str]) -> str:
#         """IMPROVED: Better query type determination"""
        
#         # Complex multi-aspect indicators (highest priority)
#         complex_indicators = [
#             'compare.*across.*companies', 'each company.*change', 'all three companies',
#             'growth rates.*across', 'compare.*all three', 'each company.*margin',
#             'percentage.*across.*companies', 'r&d.*percentage.*revenue'
...
#         # Cross-company comparison (multiple companies, single aspect)
#         elif (len(companies) > 1 and 
#               any(word in query_lower for word in ['which', 'highest', 'lowest', 'most', 'least'])):
#             return 'cross_company'
        
#         # Complex multi-aspect (multiple companies + multiple years OR complex comparison)
#         elif (len(companies) > 1 and len(years) > 1) or 'compare' in query_lower:
#             return 'complex_multi_aspect'
        
#         # YoY comparison (single company, multiple years, growth-related)
#         elif (len(companies) == 1 and len(years) > 1 and 
#               any(word in query_lower for word in ['grow', 'growth', 'change', 'from', 'to'])):
#             return 'yoy_comparison'
...
#         # AI strategy or qualitative questions
#         elif any(word in query_lower for word in ['ai risks', 'strategy', 'mentioned', 'differ']):
#             if len(companies) > 1 or 'each company' in query_lower:
#                 return 'complex_multi_aspect'
#             else:
#                 return 'simple_direct'
        
#         # Simple direct queries (default)
#         else:
#             return 'simple_direct'

#     def generate_sub_queries(self, parsed_query: Dict[str, Any]) -> List[str]:
#         """IMPROVED: Generate sub-queries based on query type and components"""
#         query_type = parsed_query['query_type']
#         companies = parsed_query['companies'] or ['MSFT', 'GOOGL', 'NVDA']
#         years = parsed_query['years'] or ['2023']
#         metrics = parsed_query['metrics'] or ['revenue']
        
#         sub_queries = []
        
#         if query_type == 'simple_direct':
#             # Single company, single metric, single year
#             company = companies[0] if companies else 'MSFT'
#             year = years[0] if years else '2023'
#             metric = metrics[0] if metrics else 'revenue'
#             sub_queries.append(f"{company} {metric} {year}")
...
#         elif query_type == 'yoy_comparison':
#             # Single company, multiple years for growth calculation
#             company = companies[0] if companies else 'MSFT'
#             metric = metrics[0] if metrics else 'revenue'
#             for year in years:
#                 sub_queries.append(f"{company} {metric} {year}")
...
#         elif query_type == 'cross_company':
#             # Multiple companies, single year, single metric
#             year = years[0] if years else '2023'
#             metric = metrics[0] if metrics else 'revenue'
#             for company in companies:
#                 sub_queries.append(f"{company} {metric} {year}")
...
#         elif query_type == 'segment_analysis':
#             # Specific analysis like percentage calculations
#             company = companies[0] if companies else 'GOOGL'
#             year = years[0] if years else '2023'
#             # Generate queries for both total and segment revenue
#             sub_queries.append(f"{company} total revenue {year}")
#             sub_queries.append(f"{company} cloud revenue {year}")
#             sub_queries.append(f"{company} advertising revenue {year}")
        
#         else:  # complex_multi_aspect - FIXED
#             # IMPROVED: Handle complex multi-aspect queries properly
            
#             # SPECIAL CASE: R&D as percentage of revenue
#             if ('r&d' in parsed_query['original_query'].lower() and 'percentage' in parsed_query['original_query'].lower()):
#                 year = years[0] if years else '2023'
#                 for company in companies:
#                     sub_queries.append(f"{company} research and development {year}")
#                     sub_queries.append(f"{company} total revenue {year}")
            
#             # For queries like "How did each company's margin change from 2022 to 2024?"
#             elif len(years) > 1 and len(companies) > 1:
#                 for company in companies:
#                     for year in years:
#                         for metric in metrics[:1]:  # Primary metric only
#                             sub_queries.append(f"{company} {metric} {year}")
            
#             # For queries like "Compare across all three companies"
...
#             # For AI risk queries (qualitative)
#             elif 'ai' in parsed_query['original_query'].lower() or 'artificial intelligence' in parsed_query['original_query'].lower():
#                 for company in ['MSFT', 'GOOGL', 'NVDA']:
#                     # Search for general AI mentions and risks
#                     sub_queries.append(f"{company} artificial intelligence 2024")
#                     sub_queries.append(f"{company} ai technology 2024")
#                     sub_queries.append(f"{company} machine learning 2024")
            
#             # Default complex handling
#             else:
#                 for company in companies[:3]:
#                     for year in years[:2]:
#                         for metric in metrics[:1]:
...
#         return sub_queries[:12]  # Increased limit for complex queries
...
#     def _synthesize_complex_answer(self, parsed_query: Dict[str, Any], sub_results: List[QueryResult]) -> Dict[str, Any]:
#         """IMPROVED: Better synthesis for complex multi-aspect queries"""
#         query_lower = parsed_query['original_query'].lower()
        
#         # Handle R&D percentage comparison - FIXED
#         if 'r&d' in query_lower and 'percentage' in query_lower:
#             return self._synthesize_rd_percentage_comparison(parsed_query, sub_results)
...
#         # Handle AI risks queries - IMPROVED
#         elif 'ai risks' in query_lower or ('ai' in query_lower and ('risks' in query_lower or 'differ' in query_lower)):
#             return self._synthesize_ai_risks(parsed_query, sub_results)
...
#     def _synthesize_ai_risks(self, parsed_query: Dict[str, Any], sub_results: List[QueryResult]) -> Dict[str, Any]:
#         """IMPROVED: AI risks comparison (qualitative analysis)"""
#         company_insights = {}
#         sources = []
...
#                     content_lower = chunk.content.lower()
#                     if any(ai_term in content_lower for ai_term in ['ai', 'artificial intelligence', 'machine learning', 'algorithm']):
                        
#                         if result.company not in company_insights:
#                             company_insights[result.company] = []
                        
#                         # Extract relevant sentences about AI
#                         sentences = chunk.content.split('.')
#                         ai_sentences = []
#                         for sentence in sentences:
#                             if any(ai_term in sentence.lower() for ai_term in ['ai', 'artificial intelligence', 'machine learning']):
#                                 ai_sentences.append(sentence.strip())
                        
#                         if ai_sentences:
#                             company_insights[result.company].extend(ai_sentences[:2])  # Limit per chunk
...
#             return {
#                 'query': parsed_query['original_query'],
#                 'answer': "Limited AI-related content found in the available financial data. SEC 10-K filings may not contain detailed AI risk discussions in the sections analyzed.",
#                 'reasoning': "Searched for AI-related content across all companies but found limited relevant information in the financial metrics data.",
#                 'sub_queries': [result.query for result in sub_results],
#                 'sources': sources[:3],
#                 'confidence': 0.3
#             }
...
#         insights_summary = []
#         for company, insights in company_insights.items():
#             unique_insights = list(set(insights))  # Remove duplicates
#             if unique_insights:
#                 summary = f"{company}: {len(unique_insights)} AI-related mentions"
#                 insights_summary.append(summary)
        
#         answer = f"AI-related information comparison: {', '.join(insights_summary)}. Note: Comprehensive AI risk analysis would require access to full 10-K text sections beyond financial metrics."
        
#         return {
#             'query': parsed_query['original_query'],
#             'answer': answer,
#             'reasoning': "Searched financial filings for AI-related content. Limited detailed risk analysis available in structured financial data.",
#             'sub_queries': [result.query for result in sub_results],
#             'sources': sources[:6],
#             'confidence': 0.5,
#             'ai_mentions': {comp: len(insights) for comp, insights in company_insights.items()}
#         }
...
#     # Include all other methods from previous version (unchanged)
#     def extract_financial_values(self, chunks: List[Any]) -> List[Dict[str, Any]]:
#         """Extract numerical values from chunks"""
#         values = []
...
#     def synthesize_answer(self, parsed_query: Dict[str, Any], sub_results: List[QueryResult]) -> Dict[str, Any]:
#         """Synthesize final answer based on query type and results"""
#         query_type = parsed_query['query_type']
...
#         return {
#             'query': parsed_query['original_query'],
#             'answer': "Unable to find relevant financial data for this query.",
#             'reasoning': "No matching financial data found in the available SEC filings.",
#             'sub_queries': [],
#             'sources': [],
#             'confidence': 0.0
#         }
